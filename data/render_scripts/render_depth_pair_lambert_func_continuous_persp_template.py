...
def run(CATEGORY, MODEL, NAME):
        ...
        for res in res_list:
            ...
            for i in range(azim_all.shape[0]):
                for j in range(elev_all.shape[0]):
                    ...
                    shutil.copyfile(scene.render.filepath,
                                    "{0}/RGB_{1}_{2}.png".format(save_path, int(azim), int(elev)))
    ...
    print(util.toGreen(
        "#{3} {1} done, time={0:.4f} sec. Model saved to {2}".format(time.time() - timeStart, MODEL, trans_path,
                                                                     BUFFER)))
